export default class Sequence {
  ...
  static updateCallbackExeConditionMap(actionName: string): {
    ...
    if (sequenceMap[actionName]) {
      activeResponseMap.emit = !(
        Object.keys(sequenceMap[actionName].responseEmitState).length > 0
      );
      activeResponseMap.broadcast = !(
        Object.keys(sequenceMap[actionName].responseBroadcastState).length > 0
      );
    }
  ...
  static convertServerToApiIoType(iFrameId: string, actionType: string) {
    if (
      actionType.indexOf(
        `${Sequence.API_SEPARATE_IO_TYPE_START}${Sequence.API_REQUEST_TYPE}${Sequence.API_SEPARATE_IO_TYPE_END}`
      ) >= 0
    ) {
    ...
    if (
      actionType.indexOf(
        `${Sequence.API_SEPARATE_IO_TYPE_START}${Sequence.API_RESPONSE_TYPE_EMIT}${Sequence.API_SEPARATE_IO_TYPE_END}`
      ) >= 0
    ) {
  ...
  static getRequestState(
    ...
    const sequenceMap = Sequence.map as any;
    const { requestPublicState, requestPrivateState } =
      sequenceMap[endpointKey];
  ...
  static getResponseState(
    ...
    const responseSchema =
      sequenceMap[endpointKey][`response${responseType}State`];
  ...
  static getRequestActionState(
    actionName: string,
    requestParams1: any = null,
    requestParams2: any = null
  ) {
    if (typeof requestParams1 === "string" && requestParams2 === null) {
      return { type: actionName };
    }
    if (
      typeof requestParams1 === "object" &&
      typeof requestParams2 === "object"
    ) {
      return { ...requestParams1, ...requestParams2, type: actionName };
    }
    if (
      typeof requestParams1 === "string" &&
      typeof requestParams2 === "object"
    ) {
