...
export default class PostMessage {
  ...
  static getMessageTypes(actionType: string): {
    ...
    const splited2 =
      splited1[0] && splited1[0] ? splited1[0] && splited1[0].split("[") : [];
    const ioType =
      splited2[1] && splited2[1]
...
export type MessageClientAndExtType = {
  id: string;
  type:
    | typeof PostMessage.EXT_TO_CLIENT_TYPE
    | typeof PostMessage.CLIENT_TO_EXT_TYPE;
  ioType: IoTypeValues;
  method: string;
  href: string;
  params?: MessageParams;
...
export type MessageMediaClientAndMediaServerType = {
  id: string;
  type:
    | typeof PostMessage.MEDIA_CLIENT_TO_MEDIA_SERVER_TYPE
    | typeof PostMessage.MEDIA_SERVER_TO_MEDIA_CLIENT_TYPE;
  method: string;
  params?: MessageParams;
  methodBack?: string;
};
