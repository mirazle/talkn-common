...
export type EnvType =
  | typeof define.DEVELOPMENT
  | typeof define.LOCALHOST
  | typeof define.PRODUCTION;
export type BootType =
  | typeof define.APP_TYPES.API
  | typeof define.APP_TYPES.CLIENT
  | typeof define.APP_TYPES.EXTENSION;
export type BootProtocolType =
  | typeof Sequence.HTTPS_PROTOCOL
  | typeof Sequence.HTTP_PROTOCOL
  | typeof Sequence.TALKN_PROTOCOL;
...
export default class BootOptionModel {
  ...
  constructor(params: Partial<BootOption> = init) {
    ...
    this.hasSlash =
      params.hasSlash || BootOptionModel.getLastHasSlach(initialRootCh);
    this.connection =
      params.connection ||
      BootOptionModel.getActiveConnection(
        initialRootCh,
        firstHasSlash,
        this.hasSlash
      );
    ...
    this.extensionMode =
      params.extensionMode || BootOptionModel.extensionModeNone;
    ...
    switch (env) {
      case define.PRODUCTION:
        if (initialRootCh.indexOf(conf.coverURL) >= 0) {
          ...
          initialRootCh = initialRootCh.replace(
  ...
  static getProtocol(): BootProtocolType {
    if (location.protocol === Sequence.HTTPS_PROTOCOL)
      return Sequence.HTTPS_PROTOCOL;
    if (location.protocol === Sequence.HTTP_PROTOCOL)
      return Sequence.HTTP_PROTOCOL;
    return Sequence.TALKN_PROTOCOL;
  }

  static getHost(params?: BootOption) {
    if (isNodeServer()) {
  ...
  static getActiveConnection(
    initialRootCh: string,
    firstHasSlash: boolean,
    lastHasSlash: boolean
  ): string {
